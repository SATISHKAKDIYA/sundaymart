...
import shopsCategoryDelete from "../../requests/ShopCategories/ShopsCategoryDelete";
...
class ShopCategories extends React.Component {
    columns = [
        ...
        },
        {
            ...
            render: (options, row) => {
                return (<div>
                    {
                        options.edit && isAllowed("/shops-categories/edit") && (
                            <Button type="link">
                                <Link to={{
                                    pathname: "/shops-categories/edit",
                    ...
                    {
                        options.delete && isAllowed("/shops-categories/delete") && row.default != 1 && (<Popconfirm
    ...
    handleOk = async (id) => {
        ...
        let data = await shopsCategoryDelete(id);
    ...
    fetch = (params = {}) => {
        ...
        reqwest({
            url: '/api/auth/shop-category/datatable',
            method: 'post',
            type: 'json',
            headers: {
        ...
        return (
            <PageLayout>
                <Breadcrumb style={{margin: '16px 0'}}>
                    <Breadcrumb.Item>Shops categories</Breadcrumb.Item>
                </Breadcrumb>
                <PageHeader
                    className="site-page-header"
                    title="Shops categories"
                    subTitle="Create, remove and edit shop categories"
                    extra={[
                        isAllowed("/shops-categories/add") && <Link to="/shops-categories/add" key="add" className="btn btn-success">
                            Add Shops category
                        </Link>
                    ]}
                >
