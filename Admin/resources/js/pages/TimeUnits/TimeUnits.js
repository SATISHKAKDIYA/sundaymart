...
import timeUnitDelete from "../../requests/TimeUnits/TimeUnitDelete";
...
class TimeUnits extends React.Component {
    columns = [
        ...
        },
        {
            ...
            render: (options, row) => {
                return (<div>
                    {
                        options.edit && isAllowed("/time-units/edit") && (
                            <Button type="link">
                                <Link to={{
                                    pathname: "/time-units/edit",
                    ...
                    {
                        options.delete && isAllowed("/time-units/delete") && row.default != 1 && (<Popconfirm
    ...
    handleOk = async (id) => {
        ...
        let data = await timeUnitDelete(id);
    ...
    fetch = (params = {}) => {
        ...
        reqwest({
            url: '/api/auth/time-unit/datatable',
            method: 'post',
            type: 'json',
            headers: {
        ...
        return (
            <PageLayout>
                <Breadcrumb style={{margin: '16px 0'}}>
                    <Breadcrumb.Item>Time Units</Breadcrumb.Item>
                </Breadcrumb>
                <PageHeader
                    className="site-page-header"
                    title="Time Units"
                    subTitle="Create, remove and edit time units"
                    extra={[
                        isAllowed("/time-units/add") &&
                        <Link to="/time-units/add" key="add" className="btn btn-success">
                            Add time units
                        </Link>
                    ]}
                >
